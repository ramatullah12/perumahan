    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden group hover:shadow-xl transition-all duration-500">
            ...
            <div class="h-64 overflow-hidden relative bg-gray-100">
                @if($tipe->gambar)
                    {{-- PERBAIKAN: Memanggil URL Cloudinary secara langsung --}}
                    <img src="{{ $tipe->gambar }}" 
                         class="w-full h-full object-cover group-hover:scale-110 transition duration-700" 
                         alt="Foto Tipe {{ $tipe->nama_tipe }}"
                ...
                <div class="absolute top-4 left-4 bg-white/90 backdrop-blur px-4 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest text-blue-600 shadow-sm border border-blue-50">
                    <i class="fas fa-building mr-1"></i> {{ $tipe->project->nama_proyek ?? 'Tanpa Proyek' }}
                </div>
            ...
            <div class="p-8">
                <div class="mb-4">
                    <div class="flex justify-between items-start">
                        ...
                        <span class="text-[10px] bg-green-50 text-green-600 px-2 py-1 rounded-md font-bold uppercase border border-green-100">
                            Unit Aktif
                        </span>
                    </div>
                    <p class="text-blue-600 font-black text-2xl mt-2 tracking-tight">
                        <span class="text-sm font-bold mr-1">Rp</span>{{ number_format($tipe->harga, 0, ',', '.') }}
                    </p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-y-4 border-t border-gray-100 pt-6 mt-6">
                    <div class="flex items-center text-gray-600 text-sm font-semibold">
                        <div class="w-9 h-9 rounded-xl bg-blue-50 flex items-center justify-center mr-3 text-blue-600">
                            <i class="fas fa-expand-arrows-alt text-xs"></i>
                        </div>
                        LT: {{ $tipe->luas_tanah }}m²
                    </div>
                    <div class="flex items-center text-gray-600 text-sm font-semibold">
                        <div class="w-9 h-9 rounded-xl bg-orange-50 flex items-center justify-center mr-3 text-orange-600">
                            <i class="fas fa-home text-xs"></i>
                        </div>
                        LB: {{ $tipe->luas_bangunan }}m²
                    </div>
                    <div class="flex items-center text-gray-600 text-sm font-semibold">
                        <div class="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center mr-3 text-purple-600">
                            <i class="fas fa-bed text-xs"></i>
                        </div>
                        {{ $tipe->kamar_tidur }} Kamar Tidur
                    </div>
                    <div class="flex items-center text-gray-600 text-sm font-semibold">
                        <div class="w-9 h-9 rounded-xl bg-cyan-50 flex items-center justify-center mr-3 text-cyan-600">
                            <i class="fas fa-bath text-xs"></i>
                        </div>
                        {{ $tipe->kamar_mandi }} Kamar Mandi
                    </div>
                </div>
            </div>
        </div>
        @empty
        <div class="col-span-full bg-white rounded-[3rem] p-24 text-center border-2 border-dashed border-gray-200">
            <div class="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6">
                <i class="fas fa-layer-group text-4xl text-gray-200"></i>
            </div>
            <h3 class="text-xl font-black text-gray-400 uppercase tracking-widest">Belum ada tipe rumah</h3>
            <p class="text-gray-400 mt-2 text-sm italic mb-8">Silakan tambahkan spesifikasi tipe rumah Anda.</p>
            <a href="{{ route('admin.tipe.create') }}" class="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-blue-600 transition-all shadow-lg">
                + Tambah Sekarang
            </a>
        </div>
        @endforelse
...
<script>
    function btnDelete(id, name) {
        Swal.fire({
            ...
            html: `Apakah Anda yakin ingin menghapus <b>${name}</b>?<br><small class="text-red-500 font-medium italic">Seluruh data unit yang terkait juga akan hilang.</small>`,
            icon: 'warning',
