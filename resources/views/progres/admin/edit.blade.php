...
<div class="p-8 bg-slate-50 min-h-screen">
    <div class="max-w-4xl mx-auto">
        ...
        <nav class="flex items-center space-x-2 text-sm font-medium mb-8">
            <a href="{{ route('admin.progres.index') }}" class="text-slate-400 hover:text-blue-600 transition-colors flex items-center">
                <i class="fas fa-arrow-left mr-2 text-xs"></i> Monitoring
            </a>
            <span class="text-slate-300">/</span>
            <span class="text-slate-900 font-bold">Update Progres Unit</span>
        </nav>
        ...
        <div class="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200/50 overflow-hidden border border-slate-100 animate-fade-in">
                ...
                <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div class="flex items-center gap-4 bg-white/5 p-4 rounded-3xl border border-white/10 backdrop-blur-md">
                        <div class="text-right">
                            ...
                            <p class="text-2xl font-black text-white leading-none">{{ $unit->progres_pembangunan ?? 0 }}%</p>
                        </div>
            ...
            {{-- Form Section --}}
            <form action="{{ route('admin.progres.update', $unit->id) }}" method="POST" enctype="multipart/form-data" class="p-10 space-y-10" id="updateForm">
                @csrf
                @method('PUT')

                @if ($errors->any())
                    <div class="bg-red-50 border-l-4 border-red-500 p-4 rounded-xl mb-6">
                        <ul class="text-red-700 text-sm font-bold">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between ml-2">
                            ...
                            <span class="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-md">Wajib</span>
                        </div>
                ...
                <div class="space-y-4">
                    ...
                    <textarea name="keterangan" rows="3" 
                              placeholder="Detail pekerjaan hari ini..." 
                        ...
                        <div class="relative">
                            ...
                            <label for="foto-input" class="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-slate-200 rounded-[2.5rem] cursor-pointer bg-slate-50 hover:bg-blue-50/50 hover:border-blue-400 transition-all duration-500 group overflow-hidden">
                                <div id="upload-placeholder" class="flex flex-col items-center justify-center py-10 transition-all">
                                    ...
                                    <p class="text-sm font-bold text-slate-600 uppercase tracking-widest">Ganti Foto</p>
                                </div>
                            ...
                            @if($unit->latestProgres && $unit->latestProgres->foto)
                                <div class="flex items-center gap-6">
                                    <div class="relative w-40 h-40 flex-shrink-0">
                                        {{-- SINKRONISASI CLOUDINARY: Tidak perlu asset('storage') jika URL full --}}
                                        <img src="{{ Str::startsWith($unit->latestProgres->foto, 'http') ? $unit->latestProgres->foto : asset('storage/' . $unit->latestProgres->foto) }}" 
                                             class="w-full h-full object-cover rounded-[2rem] shadow-xl border-4 border-white">
                                        <div class="absolute -bottom-2 -right-2 bg-slate-900 text-white p-2 rounded-xl text-[8px] font-black uppercase tracking-widest">Active</div>
                                    </div>
                                    <div class="flex-1">
                                        <h4 class="text-xs font-black text-slate-800 uppercase tracking-widest mb-1">Foto Saat Ini</h4>
                                        <p class="text-[11px] text-slate-500 leading-relaxed">Akan tetap disimpan jika Anda tidak mengunggah foto baru.</p>
                                    </div>
                                </div>
                            @else
                                <div class="text-center py-10">
                                    <i class="fas fa-images text-4xl text-slate-200 mb-3"></i>
                                    <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Belum Ada Dokumentasi</p>
                                </div>
                            @endif
                        </div>
                    </div>
